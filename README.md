...
파일을 **1,158개 청크**로 색인하고, 골든셋(**26문항** — 1~3주차 20 + 4주차 6)으로 alpha를
0.0~1.0까지 재스윕한 뒤 확정했습니다.

| 지표 | 목표 | 실제 (alpha=0.4, tokens + golden_set 복수 정답 스키마) |
|---|---|---|
| Hit Rate@5 | ≥ 0.8 | **0.962** |
| MRR | ≥ 0.5 | **0.827** |
...
`expected_chunks`(복수 허용)로 스키마를 확장했고, 그 결과 Hit Rate@5 0.923→0.962,
MRR 0.788→0.827로 다시 한번 개선됐습니다. 전체 과정과 근본 원인 분석은
...
Haiku 기준). 프롬프트 개선(Answer Relevancy 목표)과 청킹 전략 전환(tokens 채택), 두 차례
...
- [x] golden_set 스키마를 `expected_chunk`(단일) → `expected_chunks`(복수 허용)로 확장 완료 — gs-23에 정답 3개 반영, Hit Rate@5 0.923→0.962·MRR 0.788→0.827로 재개선 (`docs/adr.yaml` ADR-009)
