...
`scripts/run_ragas_evaluation.py`로 전체 파이프라인(검색+재랭킹+생성)을 실제로 돌려 계산합니다
(Anthropic Claude Haiku 기준). 골든셋이 26문항이던 시점부터 프롬프트 개선(Answer Relevancy
목표) → 청킹 전략 전환(tokens 채택) → 골든셋 커버리지 확장(41문항, alpha 0.5)까지 세 차례
변경 전후를 비교했습니다 — 전체 실험 과정은 [`docs/EXPERIMENTS.md`](docs/EXPERIMENTS.md) 참고:

| 지표 | 단계 | 최초(block, 26문항) | 프롬프트 개선 후(block, 26문항) | tokens 채택 후(26문항) | **41문항 확장 후(최종)** |
|---|---|---|---|---|---|
| Faithfulness (충실도) | 생성 | 0.877 | 0.878 | 0.958 | **0.948** |
| Answer Relevancy (답변 관련성) | 생성 | 0.784 | 0.790 | 0.859 | **0.833** |
| Context Precision (검색 정밀도) | 검색 | 0.844 | 0.844 | 0.910 | **0.910** |
| Context Recall (검색 재현율) | 검색 | 0.923 | 0.962 | 1.000 | **1.000** |

청킹 전략을 tokens로 바꿨을 때는 4개 지표 전부 뚜렷하게 개선됐습니다 — 청크당 더 완전한 문맥을
담게 되면서 검색뿐 아니라 LLM 생성 품질에도 도움이 된 것으로 해석됩니다. 반면 26→41문항으로
커버리지를 넓힌 뒤에는 Faithfulness·Answer Relevancy가 소폭 하락했는데, 이는 성능이 실제로
나빠졌다기보다 Hit Rate/MRR 때와 같은 패턴입니다 — 문항이 26개로 적을 때는 우연히 잘 맞는
소수 항목에 지표가 좌우되기 쉬웠고, 새로 추가된 15문항이 더 다양한 주제(Transformer 구조,
Tokenizer, KV Cache 등)를 건드리면서 파이프라인의 실제 성능 분포가 더 정직하게 드러난 것으로
해석합니다. 다만 두 가지는 정직하게 밝혀둡니다: (1) 41문항도 여전히 통계적으로 넉넉한 표본은
아니고, (2) 이번 재평가 중 164개 판정 작업 중 8개가 `LLMDidNotFinishException`(max_tokens
부족)으로 실패해, Context Recall=1.000처럼 극단적인 수치는 "완벽"이 아니라 "강한 신호" 정도로
해석하는 것이 정확합니다.
