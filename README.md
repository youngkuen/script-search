...
전체 파이프라인(검색+재랭킹+생성)을 실제로 돌려 RAGAS 4대 지표를 계산했습니다(총 171초, Anthropic
Claude Haiku 기준):

| 지표 | 단계 | 점수 |
|---|---|---|
| Faithfulness (충실도) | 생성 | **0.877** |
| Answer Relevancy (답변 관련성) | 생성 | **0.784** |
| Context Precision (검색 정밀도) | 검색 | **0.844** |
| Context Recall (검색 재현율) | 검색 | **0.923** |

4개 지표 모두 0.78 이상으로 뚜렷한 약점은 없습니다. 검색 단계(Context Recall 0.923)가 특히
탄탄한데, 4-2에서 추가한 Cross-Encoder 재랭킹의 기여로 보입니다. 4개 중 가장 낮은 Answer
Relevancy(0.784)는 "질문에 얼마나 직접적으로 답했는가"를 보는 지표라, 프롬프트에 "직접·간결하게
답하라"는 지시를 강화하면 개선 여지가 있습니다(6-1 강의의 진단 원칙 그대로). 26문항은 강의 권장
최소치(20개)는 넘겼지만 통계적으로 넉넉한 표본은 아니라서, 이 수치는 확정 성능이 아니라 첫
벤치마크로 보는 것이 정확합니다.
...
- [ ] Answer Relevancy(0.784, 4대 지표 중 최저) 개선 — 프롬프트에 "직접·간결하게 답하라" 지시 강화 후 재평가
